...
STOPLIMIT_CMD_FILE = "./cmds/stoplimit"
...
def isStopLimitOrderFilled(coin_name, current_price_float):
  stoplimit_price_last_float = stoplimit_price_list.get_val(coin_name)
  ...
  if(sell_order_status == "No record found"):
    ...
    if(status_order == "FILLED"):
      try:
        printMsg("STOP LIMIT order has been FILLED")
        printMsg("calculating gains...")
      ...
      printMsg("Pending SELL STOPLIMIT order with status: " + status_order)
      return False
...
def trade_execution():
  ...
  while RUN :
    ...
    if(coin_name in buy_order_list):
      ...
      printMsg("")
      printMsg("BUY increase (%): " + str(coin_price_diff_perc))
      printMsg("STOPLIMIT increase (%): " + str(coin_price_diff_perc_stoplimit))
      ...
      # check Order Status
      if(isStopLimitOrderFilled(coin_name, current_price_float)):
        if(MODE == "ONCE"):
          RUN = False
          exit_trade(0)
      ...
      # check if STOPLIMIT command has been issued
      elif(path.exists(STOPLIMIT_CMD_FILE)):
        printMsg("---------------------------------------------------------------------------")
        printMsg("STOPLIMIT command detected...")
        printMsg("getting preview stop limit order...")
        prev_sell_order = Buy_Order_List_Executed.get_val(coin_name + "_sell")
        if(prev_sell_order == "No record found"):
          printMsg("No record found")
        ...
        if(STOP_LIMIT_SELL_THRESHOLD != -1):
          printMsg("setting new STOP LIMIT order...")
          coin_price = getCoinLastPrice(coin_name)
          # reset the current stoplimit price to that the new stoplimit order is relative to the current price
          stoplimit_price_list.set_val(coin_name, "No record found")
          updateSell_STOP_LIMIT(coin_name, coin_price, STOP_LIMIT_SELL_THRESHOLD)
          try:
            os.remove(STOPLIMIT_CMD_FILE)
          except:
            printMsg("Error in removing command")       
      else:
...
if __name__ == '__main__':
    ...
    if(len(sys.argv)) > 7:
        ...
        print("###################################################")
        print("")
        print("          CryptoTraderAIHelperTool " + "V" + VERSION + "")
        print("")
        print("")
        print("")
        ...
        if(BUY_NOW_PARAM == "BUYFAST"):
          ...
          while  not coinSell:
              try:
                ...
                gain_trade_prec = coin_gain / (buy_price_list_str * current_coin_balance_int) * 100
                stoplimit_price = stoplimit_price_list.get_val(COIN_NAME_TO_TRADE)

                # real gaicumulative_price_increasen with current price
                coin_price_diff_real = coin_price - buy_price_list_str
                absolute_gain_amount = coin_price_diff_real * (MAX_AMOUNT_BUY / buy_price_list_str)

                # estimated gain considering stoplimit
                coin_price_diff = stoplimit_price - buy_price_list_str
                estimated_gain_amount = coin_price_diff * (MAX_AMOUNT_BUY / buy_price_list_str)
                ...
                printMsg("StopLimit:      " + str(stoplimit_price))
                ...
                # check Order Status
                if(isStopLimitOrderFilled(COIN_NAME_TO_TRADE, coin_price)):
                  exit_trade(0)

                # check if SELL command has been issued
                if(path.exists(SELL_CMD_FILE)):
                  coinSell = True
                  printMsg("---------------------------------------------------------------------------")
                  printMsg("SELL command detected...selling coin...")
                  printMsg("getting preview stop limit order...")
                ...
                # check if STOPLIMIT command has been issued
                  ...
                  if(STOP_LIMIT_SELL_THRESHOLD != -1):
                    ...
                    coin_price = getCoinLastPrice(COIN_NAME_TO_TRADE)
                    # reset the current stoplimit price to that the new stoplimit order is relative to the current price
                    stoplimit_price_list.set_val(COIN_NAME_TO_TRADE, "No record found")
                    updateSell_STOP_LIMIT(COIN_NAME_TO_TRADE, coin_price, STOP_LIMIT_SELL_THRESHOLD)
                    try:
                      os.remove(STOPLIMIT_CMD_FILE)
                    except:
